package com.zskjprojectj.andouclient.refresh;
...
public class MySwipeRefreshLayout extends ViewGroup implements NestedScrollingParent,
    ...
    @SuppressLint("NewApi")
    private void startDragging(float y, float x) {
        final float yDiff = y - mInitialDownY;
        final float xDiff = x - mInitialDownX;
        if (yDiff > mTouchSlop && !mIsBeingDragged && Math.abs(xDiff) <= Math.abs(yDiff)) {
